...
class Hunt {
    __New(mp, pp) {
        ...
        SetupGDIP(this.MapProps.screenSize, this.MapProps.screenSize, this.skloWindow)
    }
    ...
    FindWindow() {
        ...
        if this.skloWindow { ; Check if the window actually exists
            ...
            this.MapProps.SetXCoord(Ox) ; Set the X coordinates of the pixel
            this.MapProps.SetYCoord(Oy) ; Set the Y coordinates of the pixel
        }
    ...
    Draw() {
        ...
        pBrush := Gdip_CreatePen(this.PieProps.smallPieColorHex, 3)

        ; Draw the pie slice with the set parameters
        Gdip_DrawPie(
            graphics, ; The graphic to draw in 
            pBrush, ; The brush to use
            this.__GetAbsoluteCoord(this.MapProps.xPosition, this.PieProps.sizeSmallPie), ; The calculated absolute x coordinate point
            this.__GetAbsoluteCoord(this.MapProps.yPosition, this.PieProps.sizeSmallPie), ; the calculated absolute y coordinate point
            this.PieProps.sizeSmallPie, ; the smaller pie size is set here as the width 
            this.PieProps.sizeSmallPie, ; the smaller pie size is set here as the height
        ...
        Gdip_DrawPie(
            graphics, 
            pBrush, 
            this.__GetAbsoluteCoord(this.MapProps.xPosition, this.PieProps.sizeLargePie), ; The calculated absolute x coordinate point
            this.__GetAbsoluteCoord(this.MapProps.yPosition, this.PieProps.sizeLargePie), ; The calculated absolute y coordinate point
            this.PieProps.sizeLargePie, 
            this.PieProps.sizeLargePie, 
            this.PieProps.pieRadius + -112.5,
            45
        )
